...
const R = await p.evaluate(async () => {
  ...
  // ② まとめて動かす
  const before = selList().map(o => JSON.stringify(selObj(o).path || selObj(o).box || {x:selObj(o).x}));
  moveSel(40, 25); render(); await w(200);
  const after = selList().map(o => JSON.stringify(selObj(o).path || selObj(o).box || {x:selObj(o).x}));
...
const R2 = await p.evaluate(async () => {
  ...
  for(let i=0;i<40 && !S.pieces.length;i++) await w(100);
  const pc = S.pieces[0];
  pc.sc = 40; pc.x = S.board.w*0.4; pc.y = S.board.h*0.4; S.sel={kind:'piece',i:0};
