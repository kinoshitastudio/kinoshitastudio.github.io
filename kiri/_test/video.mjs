...
await set('r_long', 600); await set('r_n', 12000);
await new Promise(r=>setTimeout(r,700));

/* 絵の指紋（本体と同じ scatter から取る） */
const sig = () => p.evaluate(() => { const d = scatter(); let h = 2166136261;
  for(let i=0;i<Math.min(3000,d.n);i++){ h ^= (d.xs[i]*53)|0; h = Math.imul(h,16777619); h ^= (d.ys[i]*47)|0; h = Math.imul(h,16777619); }
  return (h>>>0); });
const before = await sig();
const keep = await p.evaluate(() => ({ kdir:P.kdir, seed:P.seed, mb:P.mb, n:P.n }));
...
ok(await p.evaluate(() => P.n < 26001), '再生の間は粒を減らして軽く出す', await p.evaluate(() => P.n));

/* ② 止めたら元に戻る */
await p.evaluate(() => document.getElementById('b_play').click());
await new Promise(r=>setTimeout(r,600));
const back = await p.evaluate(() => ({ kdir:P.kdir, seed:P.seed, mb:P.mb, n:P.n }));
ok(JSON.stringify(back) === JSON.stringify(keep), '止めたら値が元に戻る', JSON.stringify(back));
ok(await sig() === before, '止めたら絵も元に戻る');

/* ③ 頭と尻が同じ絵（継ぎ目なし） */
const at = u => p.evaluate(x => { setPhase(x, { kdir:P.kdir, seed:P.seed, mb:P.mb, n:P.n }); }, u);
const base = await p.evaluate(() => ({ kdir:P.kdir, seed:P.seed, mb:P.mb, n:P.n }));
const headSig = await p.evaluate(b => { setPhase(0, b); const d = scatter(); let h=2166136261;
  for(let i=0;i<Math.min(3000,d.n);i++){ h ^= (d.xs[i]*53)|0; h=Math.imul(h,16777619); } return (h>>>0); }, base);
const tailSig = await p.evaluate(b => { setPhase(1, b); const d = scatter(); let h=2166136261;
  for(let i=0;i<Math.min(3000,d.n);i++){ h ^= (d.xs[i]*53)|0; h=Math.imul(h,16777619); } return (h>>>0); }, base);
...
/* ⑤ 撮ったあと元に戻る */
const after = await p.evaluate(() => ({ kdir:P.kdir, seed:P.seed, mb:P.mb, n:P.n }));
