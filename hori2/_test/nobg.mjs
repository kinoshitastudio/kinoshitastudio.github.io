...
ok(R.地なし.透明 > 5, '地なしで撮ると外が透ける', `透明 ${R.地なし.透明}%  ${R.地なし.w}×${R.地なし.h}`);
ok(R.地なし.不透明 > 2, '地なしでも物体の所は残る（形が抜け落ちていない）', `不透明 ${R.地なし.不透明}%`);
/* 🔴 木下＝「地なしPNGで Hari に埋め込んでいるが画質悪い気もする」。
   ⭐ 原因は解像度でなく【透明な余白】。絵のあるところだけに切り詰めて出す（簾SUDARE と同じ作法）。
   ⚠️ 切り詰めないと、HARI で版面いっぱいに広げたとき余白ごと引き伸ばして粗くなる。 */
/* ⚠️⚠️ 切り詰めるだけでは【画質は上がらない】（画素の細かさは同じまま）。
   ⭐ 地なしのときは【この端末で刷れるいちばん大きい所】まで上げてから刷り、そのうえで切り詰める。
      ＝切り詰めたあとの絵が、地ありの版面と同じくらい大きいこと で見る。 */
ok(R.地なし.不透明 > 15, '絵のあるところだけに切り詰まっている', `不透明 ${R.地なし.不透明}%`);
ok(Math.max(R.地なし.w, R.地なし.h) >= Math.max(R.地あり.w, R.地あり.h) * 0.6,
   '切り詰めても十分こまかい（上限まで上げて刷っている）',
   `地あり ${R.地あり.w}×${R.地あり.h} ／ 地なし ${R.地なし.w}×${R.地なし.h}`);
