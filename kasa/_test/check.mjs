...
try{
     ...
     ⭐ 足したのは【穴も巡る】＝黒い形そのものが動く（前は光だけが動いていた） */
  {
    ...
    /* ══ 揉む ── 皺そのものが動く（2026-08-25）══
       ⭐ 揺れとの違いは【位置が動かない】こと。ここを見ないと2つの区別がつかない。
       🔴🔴 速さは「切のとき」「入のとき」だけでなく、必ず
          【一度使ってから切に戻したとき】も測る。そこが本番の状態で、
          最初の実装はそれを測らなかったせいで「一度触ると以後ずっと 25% 遅い」を見逃した。 */
    ...
    ok('揉むを入れると皺が動く（'+mv2(k0,k1)+'画素）', mv2(k0,k1)>500);
    ok('揉むも1周でぴったり戻る（'+mv2(k0,k2)+'）', mv2(k0,k2)===0);
    /* ⭐ 揺れとの違い＝位置が動かないこと。ここを見ないと2つの区別がつかない。
       ⚠️ しきいを px で固定すると、テスト用の小さい盤では相対的に大きく出て落ちる。
          盤の 2% までを「動いていない」とする（揺れは盤の 2〜3% を平行移動する＝桁が違う）。 */
    {
      ...
      ok('揉んでも位置は動かない（重心のずれ '+drift.toFixed(1)+'px＝盤の'+(drift/W2*100).toFixed(1)+'%）', drift < W2*0.02);
    }
    /* 🔴🔴 速さの回帰はここでは測らない ── このテストの盤は1コマ 2ms しか回らず、
       ブレ（±1ms）が支配して「25%悪化」のような偽の判定しか出ない。
       ⭐ 実寸（1392×1100）で別途実測した結果：
          揉むを使う前 102.3ms ／ 揉んでいる間 122.6ms ／ 0 に戻した後 98.1ms（-4.1%＝元通り）。
       ⚠️ 最初の実装は「別の配列を作って参照を差し替える」形で、一度使うと 0 に戻しても
          +24.7% のまま（リロードするまで戻らない）だった。刷るループが2種類の配列を見ると
          JIT の最適化が外れる。読む配列は【1種類】に保つこと。 */
    KASA.P.knead=knKeep;
