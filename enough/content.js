(function () {
  ...
  function setupMetrics() {
    ...
    const cssMap = {
      'x.com':       xCSS,
      'twitter.com': xCSS,
      'tiktok.com':  tiktokCSS,
      'bsky.app':    blueskyCSS,
    };

    for (const [domain, fn] of Object.entries(cssMap)) {
      if (host.includes(domain)) { if (fn) injectCSS(fn()); return; }
    }
  ...
  function setupSoundCloud() {
    injectCSS(`
      ...
      .soundTitle__playCount, .soundBadge__stats { visibility: hidden !important; }
    `);

    const hide = () => {
      // プロフィール stats（infoStats系・フォロワー数など）
