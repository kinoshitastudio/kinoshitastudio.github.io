...
## ★スライドショー / シーン切替（同じ位置に重なった全画面スライドが複数あるとき）
親フレーム直下に、**同じ位置に重なった全画面サイズの子（＝スライド）が複数**あればスライドショー。順送りで切り替える：
- 各スライドに「**登場 → 保持(1.5〜2.5秒) → 退場**」を**時間差**で割り当てる。重なりは前後0.2〜0.4秒だけ（次が入りながら前が抜ける）。1枚目は登場無しで最初から表示、最後は退場無しでもよい。
- **ただのフェードにしない＝映画的トランジションを選ぶ**：**プッシュ/スライド**(TRANSLATION_Xで画面外→0、退場は0→反対の画面外)、**ズーム/ドリー＝ケン・バーンズ**(保持中ずっと SCALE 1→1.08 でゆっくり寄る＋切替時に軽く前後移動)、**スケール＋ディゾルブ**(OPACITY 0→1 ＋ SCALE 1.05→1)、**ウィップ**(速いTRANSLATION＋EASE_IN/OUT)、**ハードカット**(OPACITYを HOLD で一瞬切替＝映画のカット)。指示があればその演出に従う。
- OPACITYで重なりの見え方を制御（保持中=1・退場後=0）。baseValueは最初のスライド以外「透明/画面外」から。
- **追加オブジェクトは作れない**＝今ある子スライドだけで構成する。全スライドを触り、時間軸で1本の流れにする。

## 原則
- 指示に沿って上質に。**『激しく/大げさに/ダイナミックに/派手に/もっと』等の指示があれば、振れ幅・移動量・回転角・スケール差・バウンド・キーフレーム数を大きく増やして思い切り誇張してよい（過剰OK・遠慮しない）**。指示が無ければ控えめに。時間は必ずトークンに合わせる。
