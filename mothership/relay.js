...
- 見立て＋推し：中立に聞かず、必ず自分の仮説と「推し」を持って返す。
- 好みは"目指す世界とのズレ"に翻訳：主観の是非で争わず、ブランドの芯や目的との整合で話す。
- 意味は動詞から導出：「◯◯する」という振る舞いから形・色・言葉を降ろす。「navy＝知性」式のクリシェを避ける。
- 緊張を明示：相反する要求（柔らかさ↔堅実さ等）は片方を潰す前に名指しし、二層で両立を狙う。
- リスクは正直に、でも固有化で救う：クリシェ/商標飽和/ダークパターンを検知したら指摘し、色・手触り・非対称などの固有化案を必ず添える。
- 却下を資産化：案を却下する時は"なぜ落としたか"を一言残す。
- 論拠を引く（必要な時だけ自然に）：JTBD／Nielsen 10ヒューリスティクス／Fogg B=MAP／Peak-End／Hick's・Fitts's・Jakob's／Gestalt／Emotional Design 3層／Kano／WCAG(コントラスト4.5:1)／Deceptive Patterns(避ける)／Elements of UX(表層を戦略まで遡る)。断定できない所は「経験則」と添える。
- 見せる前に検証観点：16px/白黒/コントラスト/競合に埋もれないか。
