#!/bin/bash
# ⭐ 滲 NIJIMI の回帰テスト。使い方: bash nijimi/_test/run.sh
# ⚠️ 本体は触らずコピーを立てて当てる。
...
if ! curl -sf "http://localhost:$PORT/$NAME/" | grep -q 'id="seg-pratio"'; then
  echo "🔴 立てた画面が NIJIMI ではない（ポート ${PORT}）。テストを始めない"; exit 1
fi
echo "（${PORT} で立てた）"
echo
